...
def likelihood(params, obs_flux, obs_flux_unc, model_flux):
    distance, radius = params
    distance2 = (distance * u.pc).to(R_sun)
    model_flux_scaled = model_flux * (radius / distance2.value)**2
...
pos = np.array([exp_distance.value.nominal_value + exp_distance.value.std_dev * np.random.randn(10),
    1.0 + 0.5 * np.random.randn(10)]).T
nwalkers, ndim = pos.shape

sampler = emcee.EnsembleSampler(nwalkers, ndim, posterior, args=(obs_flux, obs_flux_unc, model_flux), pool = multiprocessing.Pool(16), moves=emcee.moves.DEMove())
